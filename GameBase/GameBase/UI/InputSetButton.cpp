...
{
	size_ = { 100.0f, 50.0f };
	lpSceneManager.GetResourceManager().LoadDivTexture(inputIDInfo_, "Resource/resource/InputInfo.png", { 4,1 }, { 163,47 });
	lpSceneManager.GetResourceManager().LoadTexture(button_, "Resource/resource/InputSetButton.png");
	lpSceneManager.GetResourceManager().LoadTexture(frame_, "Resource/resource/InputSetButtonFrame.png");

	if (lpSceneManager.GetController().GetControllerType() == ControllerType::Key)
	{
		lpSceneManager.GetResourceManager().LoadDivTexture(inputInfo_, "Resource/resource/KeyInfo.png", { 4,12 }, { 219,47 });
		drawNowCode_ = &InputSetButton::DrawNowKeyCode;
	}
	else
	{
		auto& ctrl = static_cast<Pad&>(lpSceneManager.GetController());
		...
		{
			lpSceneManager.GetResourceManager().LoadDivTexture(inputInfo_, "Resource/resource/XboxInfo.png", { 5,2 }, { 169,47 });
...
void InputSetButton::Loaded(void)
{

...
void PlayerAttackBehavior::Update(BaseScene& scene, ObjectManager& objectManager, float delta, Controller& controller)
{
	...
	if (player.IsActive())
	{
		transform_->SetRotation({ 0.0f,0.0f,-player->GetRotation().ToEuler().y });
	}
...
void PlayerAttackBehavior::Effect(ObjectManager& objectManager)
{
	...
	for (auto& e : effect_)
	{
		...
		if (flag)
		{
			...
			auto id = objectManager.CreateFromFactory(factory, objectManager.GetPlayerID(), pos, { 0.0f,player->GetRotation().ToEuler().y ,0.0f });
...
void PlayerAttackBehavior::AddEffect(FactoryID id, ObjectManager& objectManager)
{
	// エフェクトの種類取得

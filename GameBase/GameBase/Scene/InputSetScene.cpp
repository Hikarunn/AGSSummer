	...
	step_{ 0.0f }
{
	...
	lpSceneManager.GetResourceManager().LoadTexture(mess_, "Resource/resource/InputSetMess.png");
	lpSceneManager.GetResourceManager().LoadTexture(mess2_, "Resource/resource/InputSetMess2.png");
	lpSceneManager.GetResourceManager().LoadTexture(mess3_, "Resource/resource/InputSetMess3.png");
...
void InputSetScene::UpdateWait(float delta, Controller& controller)
{
	...
	{
		step_ = 0.0f;
		if (controller.GetControllerType() == ControllerType::Key)
		{
			// キーボードの時
